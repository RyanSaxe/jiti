...
from jiti.core.declaration import Declaration, compare_signatures, short_hash
...
class JitiStore:
    ...
    def _load(self, path: Path, declaration: Declaration) -> types.FunctionType:
        ...
        fn = namespace[declaration.name]
        diff = compare_signatures(declaration, fn)
        if diff:
            raise JitiError(
                f"Signature drift in {declaration.key}: the loaded impl no longer matches "
                f"the spec.\n{diff}\n\n"
                f"Either restore the signature in {path} or delete the section to regenerate."
            )
        return fn
